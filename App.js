...
function App() {
    ...
    var _o2 = useState('default'), activeOcrPromptMode = _o2[0], setActiveOcrPromptMode = _o2[1];
    ...
    var handleRotateKey = function () { var next = GEMINI_KEYS.length > 0 ? (activeKeyIndex + 1) % GEMINI_KEYS.length : 0; setActiveKeyIndex(next); };
    ...
    return (React.createElement("div", { className: "min-h-screen transition-colors duration-700 font-sans ".concat(getBg()) },
        React.createElement("main", { className: "flex-grow flex flex-col items-center justify-center min-h-screen" }, !state.isReading ? (React.createElement(Editor, { onStart: handleStartReading, onPdfUpload: handleStartPdfReading, initialValue: state.rawText, initialPdfFile: pdfFile, initialPdfRange: pdfRange, history: history, onRestoreHistory: handleRestoreHistory, activeStructureModel: activeStructureModel, structureMode: structureMode, boldingLevel: boldingLevel, onOpenSettings: function () { return setIsSettingsOpen(true); }, googleApiKey: googleApiKey, onRotateKey: handleRotateKey })) : (React.createElement(Reader, { text: state.rawText, pdfFile: pdfFile, pdfRange: pdfRange, initialExtractedPages: extractedCache, initialPageIndex: lastPageIndex, activeModel: activeReaderModel, activeStructureModel: activeStructureModel, structureMode: structureMode, onModelChange: setActiveReaderModel, activeScanningModel: activeScanningModel, activeOcrPromptMode: activeOcrPromptMode, readerTheme: readerTheme, boldingLevel: boldingLevel, lineSpacing: lineSpacing, boldness: boldness, autoScrollSpeed: autoScrollSpeed, autoPlayTTS: autoPlayTTS, interlinearTextColor: interlinearTextColor, onBack: handleExitReader, onOpenSettings: function () { return setIsSettingsOpen(true); }, googleApiKey: googleApiKey, onRotateKey: handleRotateKey, initialHighlights: cachedHighlights, initialRewrittenPages: cachedRewrittenPages, initialXrayCache: cachedXrayCache, onStateChange: handleReaderStateChange }))),
        React.createElement(Settings, { isOpen: isSettingsOpen, history: history, onSelectHistory: function (item) { handleRestoreHistory(item); setIsSettingsOpen(false); }, activeReaderModel: activeReaderModel, activeStructureModel: activeStructureModel, structureMode: structureMode, activeScanningModel: activeScanningModel, activeOcrPromptMode: activeOcrPromptMode, readerTheme: readerTheme, boldingLevel: boldingLevel, lineSpacing: lineSpacing, boldness: boldness, autoPlayTTS: autoPlayTTS, apiKey1: userApiKey, apiKey2: GEMINI_KEYS[1], activeKeyIndex: activeKeyIndex, autoScrollSpeed: autoScrollSpeed, interlinearTextColor: interlinearTextColor, onSelectReaderModel: setActiveReaderModel, onSelectStructureModel: setActiveStructureModel, onSelectStructureMode: setStructureMode, onSelectScanningModel: setActiveScanningModel, onSelectOcrPromptMode: setActiveOcrPromptMode, onSelectTheme: setReaderTheme, onSelectBoldingLevel: setBoldingLevel, onSetLineSpacing: setLineSpacing, onSetBoldness: setBoldness, onToggleAutoPlayTTS: function () { return setAutoPlayTTS(!autoPlayTTS); }, onSetApiKey1: function(val){ setUserApiKey(val); localStorage.setItem('user_gemini_api_key', val); }, onSetApiKey2: function(){}, onSetAutoScrollSpeed: setAutoScrollSpeed, onSetInterlinearTextColor: setInterlinearTextColor, onClose: function () { return setIsSettingsOpen(false); } })));
}
